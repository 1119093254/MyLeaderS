 ...
 */
public class LayoutPopup extends PopupWindow {
	private static final String TAG = LayoutPopup.class.getSimpleName();
...
//	private int popupGravity = Gravity.NO_GRAVITY;
	...
	OnClickListener onclick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			switch (v.getId()) {
			case R.id.applauseText:
				mItemOnClickListener.onItemClick(mActionItems.get(1), 1);
				break;
			case R.id.applauseCommentLayout:
	...
	public LayoutPopup(Context context) {
		// 设置布局的参数
		this(context, LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
	}

	public LayoutPopup(Context context, int width, int height) {
...
//		mScreenHeight = Util.getScreenHeight(mContext);
		...
		// 设置弹窗的布局界面
		View view = LayoutInflater.from(mContext).inflate(R.layout.popuplayout, null);
		setContentView(view);
		Log.i(TAG," -> " + view.getHeight() + "    " + view.getWidth());
		
		priase = (TextView) view.findViewById(R.id.applauseText);
		comment = (RelativeLayout) view.findViewById(R.id.applauseCommentLayout);

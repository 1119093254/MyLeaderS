...
public class MyEditText extends EditText {
    ...
    public MyEditText(Context context, AttributeSet attrs) {
        ...
        pasColor = a.getColor(R.styleable.PasswordInputView_pivPasColor, pasColor);
        a.recycle();
        ...
        passwordPaint.setColor(pasColor);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        borderPaint_.setColor(pasColor);
        borderPaint_.setStrokeWidth((float) 10.0);
        borderPaint_.setStyle(Paint.Style.FILL);
        int number=4;//几个
        for (int i = 1; i <=number; i++) {
            float x = width * i / number;
            float x_ = width/(number*4);
            canvas.drawLine(x-x_*3,height/2, x-x_, height/2, borderPaint_);
           /* if(i<passwordLength) {
                canvas.drawLine(x, 0, x+10, height, borderPaint);
            }*/
        }

        // 密码
        float cx, cy = height/ 2;
        float half = width / number / 2;
        for(int i = 0; i < textLength; i++) {
            cx = width * i / number + half;
            float x = width / number/4;
            canvas.drawCircle(cx, cy, x+2, passwordPaint);
        }
    }

...
//                gridWidth = gridHeight = (totalWidth - gridSpacing * (columnCount - 1)) / columnCount;
    ...
    public void setAdapter(@NonNull NineGridViewAdapter adapter) {
        ...
        if (mImageInfo == null) {
            for (int i = 0; i < imageCount; i++) {
                ...
                if (iv == null) return;
               /* if(mAdapter.getImageInfo().get(0).getIsVideo()==1){
                    iv.setBitmsp(true);
                } else {
                    iv.setBitmsp(false);
                }*/
        ...
        if(adapter.getImageInfo().size()==1){
            Log.e("size",""+adapter.getImageInfo().size());
            if(adapter.getImageInfo().get(0).getIsVideo()==1){
                View child = getChildAt(0);
                if (child instanceof NineGridViewWrapper) {
                    NineGridViewWrapper imageView = (NineGridViewWrapper) child;
                    imageView.setMoreNum(-1);
                }
            } else {
                View child = getChildAt(0);
                if (child instanceof NineGridViewWrapper) {
    ...
    private NineGridViewWrapper getImageView(final int position) {
        ...
        if (position < imageViews.size()) {
            imageView = (NineGridViewWrapper) imageViews.get(position);
        } else {

...
/**
 * 作用：重写ExpandListView，为了适应购物车商品左滑可以选择删除或收藏；需要对item是左滑或上下滑动进行判断
 * Author：张凯  on 2016/7/26 16:02
 * Blog: bukevin@github.io
 */
public class SwipeLayout_Views extends LinearLayout {
    ...
    public SwipeLayout_Views(Context context) {
        super(context);
    }

    public SwipeLayout_Views(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    static List<SwipeLayout_Views> swipelayouts = new ArrayList<>();

    public static void addSwipeView(SwipeLayout_Views v) {
        if (null == v) {
            return;
        }
        swipelayouts.add(v);
    }
    public void hintSwipeView(SwipeLayout_Views v) {
        v.SimulateScroll(SHRINK);
    }
    public static void removeSwipeView(SwipeLayout_Views v) {
        if (null == v) {
            return;
        }
        v.SimulateScroll(SwipeLayout_Views.SHRINK);
    }

    private void shrinkAllView() {
        for (SwipeLayout_Views s : swipelayouts) {
            if (null == s) {
                swipelayouts.remove(s);
                continue;
            } else {
                s.SimulateScroll(SwipeLayout_Views.SHRINK);
            }

...
import com.multshows.Views.MovieRecorderView;
...
public class VideoRecording_Activity extends AppCompatActivity {
        ...
        mShootBtn.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (mRecorderView.getTimeCount() > 0)
                        handler.sendEmptyMessage(1);
                    else {
                        if (mRecorderView.getmVecordFile() != null)
                            mRecorderView.getmVecordFile().delete();
                        mRecorderView.stop();
                        Toast.makeText(VideoRecording_Activity.this, "视频录制时间太短", Toast.LENGTH_SHORT).show();
                    }
                }
                return true;
            }
    ...
    private void finishActivity() {
        if (isFinish) {
            ...
            Log.d("TAG",mRecorderView.getmVecordFile().getAbsolutePath());
            intent.putExtra("path", mRecorderView.getmVecordFile().getAbsolutePath());
            Bitmap bitmap = Utils.createVideoThumbnail(mRecorderView.getmVecordFile().getAbsolutePath());
            BitmapDrawable drawable = new BitmapDrawable(bitmap);
            drawable.setTileModeXY(Shader.TileMode.REPEAT , Shader.TileMode.REPEAT);
            drawable.setDither(true);
            Photo_ImageItem_Beans beans=new Photo_ImageItem_Beans();
            beans.setBitmap(bitmap);
            beans.setVideo(true);
            beans.setImagePath(mRecorderView.getmVecordFile().getAbsolutePath());
            Photo_Bimp.albumSelectBitmap.add(beans);

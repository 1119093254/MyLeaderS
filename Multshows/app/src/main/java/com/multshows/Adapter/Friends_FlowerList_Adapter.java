 ...
 * 送我鲜花列表 适配器
 */
public class Friends_FlowerList_Adapter extends BaseAdapter {
    //上下文
    Context mContext;
    //绑定布局类
    LayoutInflater mInflater;
    //数据集合
    List<ShowsFlower> mList;
    SaveSharedPreferences saveSharedPreferences = new SaveSharedPreferences();

    public Friends_FlowerList_Adapter(Context context, List<ShowsFlower> list) {
        mContext = context;
        mList = list;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        mViewHolder.mImageView.setImageResource(R.drawable.money_flower);
        ShowsFlower beans=mList.get(i);
        ...
        mViewHolder.mSendTime.setText(Time_Now.getStringTime(beans.getRecordTime()));

        //mViewHolder.mImageView.setImageResource(R.drawable.);
        mViewHolder.mToUserName.setVisibility(View.GONE);
        mViewHolder.mTextView.setText("送了你"+beans.getAmount()+"朵鲜花");
